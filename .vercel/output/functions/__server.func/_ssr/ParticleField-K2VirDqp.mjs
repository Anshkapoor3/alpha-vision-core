...
//#region node_modules/.nitro/vite/services/ssr/assets/ParticleField-K2VirDqp.js
...
function ParticleField() {
	...
	(0, import_react.useEffect)(() => {
		...
		let particles = [];
		const seed = () => {
			const count = Math.min(120, Math.round(width * height / 16e3));
		...
		const draw = () => {
			...
			for (let i = 0; i < particles.length; i++) {
				...
				for (let j = i + 1; j < particles.length; j++) {
					...
					if (d < 130) {
						ctx.strokeStyle = `rgba(${line}, ${(1 - d / 130) * .16})`;
						ctx.lineWidth = .6;
						ctx.beginPath();
						ctx.moveTo(a.x, a.y);
						ctx.lineTo(b.x, b.y);
